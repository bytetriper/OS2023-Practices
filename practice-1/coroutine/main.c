...
int test_multithread()
{
    ...
    for (int i = 0; i < CNT; ++i)
    {
        ...
        // printf("Thread created with ret: %d\n", ret);
        if (ret)
        {
    ...
    for (int i = 0; i < CNT; ++i)
    {
        // printf("Joining thread: %d,pthread[i]=%ld\n", i,threads[i]);
        pthread_join(threads[i], NULL);
    }
    assert(total_coroutine_count == CNT * 200);
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        if (coroutine[i] != i)
            fail("Coroutine ID not equal", __func__, __LINE__);
    }
    ...
    if (coroutine[0] != 10)
        fail("Nested coroutine ID not equal", __func__, __LINE__);
    if (co_getret(coroutine[0]) != 200)
        fail("Nested coroutine return value failed", __func__, __LINE__);
    // test nested and get status
    for (int i = 0; i < 12; ++i)
        if (co_status(i) != FINISHED)
            fail("Coroutine failed at status error", __func__, __LINE__);
